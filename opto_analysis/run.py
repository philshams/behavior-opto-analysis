...
from opto_analysis.visualize.visualization import Visualization
...
def visualize_data():
    ...
    for data_entry in selected_sessions_data_entries:
        session = create_session(data_entry, create_new=False)
        visualize = Visualization(session)
        if visualization_settings.visualize_laser_trials:
            visualize.trials(stimulus_type = 'laser', seconds_before=3, seconds_after=6)
        if visualization_settings.visualize_escape_trials: 
            visualize.trials(stimulus_type = 'audio', seconds_before=3, seconds_after=2)
