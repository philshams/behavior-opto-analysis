...
class Process():
    def __init__(self, data_entry):
        ...
        self.save_file = os.path.join(self.session.file_path, "metadata")
    ...
    def create_session(self) -> Session:        
        ...
        self.save_session(self.session, overwrite=True)
    ...
    def save_session(self, overwrite=True):
        assert not Path(self.save_file).is_file() or overwrite, "Permission to save not granted"
        with open(self.save_file, "wb") as dill_file: pickle.dump(self.session, dill_file)

    def load_session(self) -> Session:
        with open(self.save_file, "rb") as dill_file: session = pickle.load(dill_file)
        return session

    def load_registration_transform(self) -> object:
        if Path(self.save_file).is_file and isinstance(self.load_session(self.save_file).video.registration_transform, np.ndarray):
            self.loaded_registration_transform = self.load_session(self.save_file).video.registration_transform
