...
class Track():
    ...
    def create_array_with_dlc_tracking_data(self, session):
        ...
        for i, body_part in enumerate(self.tracking_data['bodyparts']):
            for j, axis in enumerate(['x', 'y']):
                self.dlc_tracking_data_array[:, i, j] = self.dlc_output[self.dlc_network_name][body_part][axis].values
            self.dlc_tracking_data_array[:, i, 2] = self.dlc_output[self.dlc_network_name][body_part]['likelihood'].values
    ...
    def fisheye_correct_tracking_data(self, session):
        if tracking_settings.inverse_fisheye_correction_file:
            ...
            self.fisheye_corrected_tracking_data_array = \
                    inverse_fisheye_map[self.dlc_tracking_data_array[:,:,1].astype(np.uint16) + session.video.y_offset,     \
            ...
            self.fisheye_corrected_tracking_data_array = self.dlc_tracking_data_array[:,:,:2]
    ...
    def plot_filtered_and_registered_tracking(self):
        if tracking_settings.display_tracking_output:
            for axis in [0,1]:
                plt.figure()
    ...
    def compute_avg_bodypart_locations(self):
        #! This section must be redone if different body parts are used during DeepLabCut tracking
    ...
    def compute_speed(self, session):
        ...
        self.tracking_data['speed'] = smoothed_speed_cm_per_sec
